...
SYNC_MARGIN = 3.
CREEP_SPEED = float(int(Params().get("Creep_Speed_Scc", encoding="utf8"))) * CV.KPH_TO_MS
...
class SccSmoother:
  ...
  def get_apply_accel(self, CS, sm, accel, stopping):
    ...
    tenesigasfactor = tenesigasfactor + gas_factor
    if CS.out.vEgo > creepspeedstart and CS.out.vEgo < 40: # 시속40까지는 가속력을 높인다.
      gas_factor = tenesigasfactor
